...
var transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: 'adityamarathe2023@gmail.com',
      pass: 'vmve njoj myjw ruiw'
    } 
  });
...
router.post('/registerdata',function(req,res){
    var dob=[];
    data=req.body.college_pnr;    //data stores PNR no
    console.log(data);
    account_address=req.body.account_address;     //stores metamask acc address
    //console.log(data);
    let sql = "SELECT * FROM pnr_info WHERE pnrno = ?" ;   
    conn.query(sql, data, (error, results, fields) => {
        ...
        if (is_registerd!='YES')
        {
          ...
          {
            var mailOptions = {
                from: 'evotingdypcetcse@gmail.com',
                to: email,
        ...
        conn.query(sql,record, function(err2,res2)
          {
             ...
             throw err2;
                ...
                var sql1="Update pnr_info set Is_registered=? Where pnrno=?";
